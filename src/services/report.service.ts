...
export class ReportService {
  async getReportPatients(organizationId: string) {
    ...
    return {
      active: patients.filter((patient: any) => String(patient.status).toLowerCase() === 'active'),
      discharged: patients.filter((patient: any) => String(patient.status).toLowerCase() === 'discharged'),
    };
  }

  async getFullPatientReport(patientId: string, organizationId: string) {
    ...
    const [vitalsRes, sleepRes, alertsRes, fallsRes, doctorNotesRes, nurseNotesRes, reportsRes] =
      await Promise.all([
        supabase
          .from('patient_vitals')
          .select('*')
    ...
    if (fallsRes.error) throw new Error(fallsRes.error.message);
    if (doctorNotesRes.error) throw new Error(doctorNotesRes.error.message);
  ...
  async getPatientCsv(patientId: string, organizationId: string) {
    ...
    (report.vitals ?? []).forEach((vital: any) => {
      rows.push(
        `Vitals,${escapeCsv(vital.heart_rate)},${escapeCsv(vital.spo2)},${escapeCsv(vital.temperature)},${escapeCsv(
          vital.respiration_rate,
        )},${escapeCsv(vital.created_at)}`,
      );
    });
    rows.push('');
    rows.push('Sleep,Sleep Duration,Deep Sleep,Score,Restlessness,Date');
    (report.sleep ?? []).forEach((sleep: any) => {
      rows.push(
        `Sleep,${escapeCsv(sleep.sleep_duration)},${escapeCsv(sleep.deep_sleep_duration)},${escapeCsv(
          sleep.sleep_score,
        )},${escapeCsv(sleep.restlessness_index)},${escapeCsv(sleep.date)}`,
      );
    });
    rows.push('');
    ...
    (report.alerts ?? []).forEach((alert: any) => {
      rows.push(
        `Alerts,${escapeCsv(alert.alert_type)},${escapeCsv(alert.severity)},${escapeCsv(alert.message)},${escapeCsv(
          alert.created_at,
        )},${escapeCsv(alert.resolved_at)}`,
      );
    });
    rows.push('');
    rows.push('Falls,Event Time,Severity,Resolved By,Notes');
    (report.falls ?? []).forEach((fall: any) => {
      rows.push(
        `Falls,${escapeCsv(fall.event_time)},${escapeCsv(fall.severity)},${escapeCsv(fall.resolved_by)},${escapeCsv(
          fall.notes,
        )}`,
      );
    });
    ...
    (report.doctorNotes ?? []).forEach((note: any) => {
      rows.push(
        `Doctor Notes,${escapeCsv(note.doctor_id)},${escapeCsv(note.created_at)},${escapeCsv(note.note)}`,
      );
    });
    rows.push('');
    rows.push('Nurse Notes,Nurse,Date,Note');
    (report.nurseNotes ?? []).forEach((note: any) => {
      rows.push(
        `Nurse Notes,${escapeCsv(note.nurse_id)},${escapeCsv(note.created_at)},${escapeCsv(note.note)}`,
      );
    });
    ...
    if (String(report.patient.status).toLowerCase() === 'discharged') {
      ...
      rows.push(
        `Discharge Summary,${escapeCsv(report.patient.discharge_date)},${escapeCsv(
          report.patient.final_condition,
        )},${escapeCsv(report.patient.discharge_summary)},${escapeCsv(report.patient.doctor_remarks)}`,
      );
    }
